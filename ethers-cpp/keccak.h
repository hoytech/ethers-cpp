...
//
// Converted into header-only library by Doug Hoyte, 2021
...
private:
  ...
  static constexpr uint64_t XorMasks[KeccakRounds] =
  {
...
static inline std::string keccak256(std::string_view input) {
  EthersCpp::Keccak keccak;
  return keccak(input.data(), input.size());
}
